...
@connect(({ positionManage, loading }) => ({
  positionManage,
  loading: loading.effects['positionManage/create'] || loading.effects['positionManage/edit'],
}))
class PositionCreateEdit extends Component {
  ...
  handleSubmit = e => {
    ...
    this.props.form.validateFields((err, values) => {
      ...
      if (!err) {
        dispatch({
          type: flag === 'create' ? 'positionManage/create': 'positionManage/edit',
          payload: values
        })
          .then(() => {
            message.success(flag === 'create' ? '添加职位成功！': '修改职位成功！')
            this.props.history.replace('/home/positionManage/list');
